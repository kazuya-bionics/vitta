...
const Nosotros = () => {
  ...
  const companyData = {
    ...
    values: [
      ...
      { icon: FaHeart, title: "Pasión", description: "Amamos lo que hacemos y se refleja en cada cliente realizado" },
    ],
  }
    ...
    <main>
      ...
      <div className="h-screen flex flex-col gap-y-4 justify-center items-center relative z-10 overflow-hidden">
        ...
        <div className="relative flex flex-col items-center justify-center gap-y-4 z-20 text-white p-8 text-center">
          <h3 className="text-6xl font-bold uppercase">¿Por qué Vitta es lo mejor para ti?</h3>
          <h3 className="text-sm opacity-80">
            Vitta está comprometido con una política justa y transparente, mira todo lo que Vitta tiene para ti.
          </h3>
          <Button styles="p-1 text-sm flex text-white/80 justify-center items-center border-white hover:border-white/75 bg-black rounded-[45px] border-2 w-[100px]" soyPublico={true}>
            Cotiza Ahora
          </Button>
        </div>
      </div>
      ...
      <section ref={sectionRef} className="w-full py-16 md:py-24 lg:py-32 bg-gradient-to-br from-white to-slate-50">
        <div className="container mx-auto px-4 md:px-6 lg:px-8 max-w-7xl">
          ...
          <div className={`text-center mb-16 transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold text-slate-900 mb-6 tracking-tight">{companyData.title}</h2>
            ...
            <div className={`space-y-8 transition-all duration-1000 delay-500 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 translate-x-8"}`}>
              ...
              <div className="pt-4">
                <button className="bg-slate-900 text-white px-8 py-4 rounded-full font-medium hover:bg-slate-800 transition-all duration-300 hover:shadow-lg hover:shadow-slate-900/25 hover:scale-105">
                  <a className="inline-flex items-center gap-3" href="https://www.facebook.com/profile.php?id=61579043823512" target="_blank" rel="noopener noreferrer">
          ...
          <div className={`transition-all duration-1000 delay-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            <div className="text-center mb-12">
              ...
              <p className="text-lg text-slate-600 max-w-2xl mx-auto">Los principios que guían nuestro trabajo y definen nuestra cultura empresarial.</p>
            </div>
